...
public  class H264Options extends AttributeContainer implements KvmSerializable
{
    ...
    public java.util.ArrayList< Enums.H264Profile> H264ProfilesSupported =new java.util.ArrayList<Enums.H264Profile >();
    ...
    protected boolean loadProperty(PropertyInfo info,SoapObject soapObject,ExtendedSoapSerializationEnvelope __envelope)
    {
        ...
        if (info.name.equals("H264ProfilesSupported"))
        {
            if(obj!=null)
            {
                if(this.H264ProfilesSupported==null)
                {
                    this.H264ProfilesSupported = new java.util.ArrayList< Enums.H264Profile>();
                }
                java.lang.Object j =obj;
                Enums.H264Profile j1= Enums.H264Profile.fromString(j.toString());
                this.H264ProfilesSupported.add(j1);
            }
            return true;
        }
        return false;
    ...
    public java.lang.Object getProperty(int propertyIndex) {
        ...
        else if(propertyIndex>=3+this.ResolutionsAvailable.size() && propertyIndex < 3+this.ResolutionsAvailable.size()+this.H264ProfilesSupported.size())
        {
            Enums.H264Profile H264ProfilesSupported = this.H264ProfilesSupported.get(propertyIndex-(3+this.ResolutionsAvailable.size()));
            return H264ProfilesSupported!=null?H264ProfilesSupported.toString():SoapPrimitive.NullSkip;
        }
        else if(propertyIndex>=3+this.ResolutionsAvailable.size()+this.H264ProfilesSupported.size() && propertyIndex < 3+this.ResolutionsAvailable.size()+this.H264ProfilesSupported.size()+this.any.size())
        {
            return this.any.get(propertyIndex-(3+this.ResolutionsAvailable.size()+this.H264ProfilesSupported.size())).getValue();
        }
        return null;
    }
    ...
    public int getPropertyCount() {
        return 3+ResolutionsAvailable.size()+H264ProfilesSupported.size()+ any.size();
    }
    ...
    public void getPropertyInfo(int propertyIndex, Hashtable arg1, PropertyInfo info)
    {
            ...
            else if(propertyIndex>=3+this.ResolutionsAvailable.size() && propertyIndex < 3+this.ResolutionsAvailable.size()+this.H264ProfilesSupported.size())
            {
                ...
                info.name = "H264ProfilesSupported";
                info.namespace= "http://www.onvif.org/ver10/schema";
            }
            else if(propertyIndex>=3+this.ResolutionsAvailable.size()+this.H264ProfilesSupported.size() && propertyIndex < 3+this.ResolutionsAvailable.size()+this.H264ProfilesSupported.size()+this.any.size())
            {
                PropertyInfo j=this.any.get(propertyIndex-(3+this.ResolutionsAvailable.size()+this.H264ProfilesSupported.size()));
                info.type = j.type;
                info.name = j.name;
